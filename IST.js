                    ...
                    async.each([0, 1], (status, next_status) => {
                        ...
                        async.retry(max_retries, (done) => {
                            if (tries) console.log(`[retrying#${tries}] ${base_url}`.yellow.bold);
                                ...
                                (error, response, body) => {
                                    if (error || !body || body.length < min_page_size) { // В случае ошибки, отсутствия данных или слишком мало данных, то пробуем ещё раз
                                        return done(true);
                                    }

                                    const obj = JSON.parse(body);

                                    if (!obj.result || !obj.result.data) { // В случае, если данные не парсятся или не приходит ответ
                                        return done(true);
                                    }
